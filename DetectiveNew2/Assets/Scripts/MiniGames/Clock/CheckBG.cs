using UnityEngine;
public class CheckBG : MonoBehaviour { public GameObject clock; public InventoryItem InventoryItem;
    ...
    public void Check() { for (int i = 0; i < playerInventory.myInventory2.Count; i++) {
            if (playerInventory.myInventory2[i].nameOfItem == "bgPath1") { clock.SetActive(true);
