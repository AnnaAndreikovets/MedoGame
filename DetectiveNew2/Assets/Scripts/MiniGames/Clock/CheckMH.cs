using UnityEngine;
public class CheckMH : MonoBehaviour { public GameObject minuteHand; public InventoryItem InventoryItem;
    ...
    public void Check() { for (int i = 0; i < playerInventory.myInventory2.Count; i++)  {
            if (playerInventory.myInventory2[i].nameOfItem == "minuteHand")  {
                minuteHand.SetActive(true); for (int p = 0; p < inventory.isFull.Length; p++){ if (p == i) {
